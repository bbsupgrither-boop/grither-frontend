...
interface ShopPageWithTabsFixedProps {
...
export function ShopPageWithTabsFixed({ 
  onNavigate, 
  currentPage, 
  onOpenSettings, 
  profilePhoto,
  ...
  const checkout = () => {
    ...
    if (userBalance >= totalPrice && cart.length > 0) {
      ...
      // Очищаем корзину и п��реключаемся на таб активных заказов
      clearCart();
      setCartTab('active');
      ...
      {shopItems.length > 0 ? (
        ...
        <div className="glass-card rounded-2xl p-8 text-center apple-shadow">
          ...
          <p className="text-sm text-muted-foreground">
            Администраторы работают над пополнением ассортимента
          </p>
        </div>
      )}
    </div>
  ...
  return (
    <div className="min-h-screen bg-background">
      <Header 
        ...
        user={{ name: 'Пользователь', username: '@user', isOnline: true }}
      />
      ...
      {isCartOpen && (
        <div className="fixed inset-0 z-50 flex items-end justify-center bg-black/50 backdrop-blur-sm">
          <div className="w-full max-w-md bg-background rounded-t-2xl max-h-[75vh] flex flex-col">
            ...
            {/* Итого и к��опки - фиксированные внизу */}
      ...
      {/* BottomNavigation скрыта когда открыта корзина */}
      {!isCartOpen && (
