# ПОЛНЫЙ КОД АДМИН ПАНЕЛИ GRITHER

## Основные файлы админ панели:

### 1. AdminPanel.tsx (Главный компонент)
```tsx
...
### 2. SettingsModal.tsx (Вход в админку)
```tsx
import { useState, useEffect } from 'react';
...
### 3. Все остальные админские компоненты должны использовать правильные стили:

**Ключевые CSS классы для админки:**
- `glass-card` - для карточек
- `apple-shadow` - для теней
- `bg-background` - для фонов
- `text-foreground` - для основного текста
- `text-muted-foreground` - для второстепенного текста
- `border-border/20` - для границ

**Важные стили из globals.css:**
```css
.glass-card {
  background: #FFFFFF;
  border-radius: 20px;
...
Все файлы должны использовать эти стили для корректного отображения в Telegram Web App.